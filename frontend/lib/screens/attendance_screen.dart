...
class _AttendanceScreenState extends State<AttendanceScreen> {
  ...
  Future<void> _loadExistingAttendances() async {
    ...
    await provider.fetchAttendance(groupId: widget.group.id, date: today); // Explicit fetch before get
    final attendances = await provider.getAttendances(widget.group.id, today);
    DateTime? maxUpdatedAt;
    Attendance? lastAttendance;

    for (var att in provider.attendanceList.where((a) => a.groupId == widget.group.id && a.date.toIso8601String().split('T')[0] == today)) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          if (lastUpdatedByName != null && lastUpdatedByRole != null && lastUpdatedAt != null)
            Padding(
  ...
  Future<void> _saveAttendance() async {
    ...
    for (var entry in _statuses.entries) {
      ...
      var data = {
        ...
        'date': nowIso,
        'updatedBy': authProvider.user!.id, // Убрали fallback, assume not null
      };
    ...
    // Reload the last updated info after save
    await _loadExistingAttendances();

    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(success ? 'Посещаемость сохранена ✅' : 'Ошибка сохранения: ${provider.error ?? 'Неизвестная ошибка'}'),
